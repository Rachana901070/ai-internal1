...
import StarWidget from "../components/Ratings/StarWidget.jsx";
import RatingBadge from "../components/Ratings/RatingBadge.jsx";
import Histogram from "../components/Ratings/Histogram.jsx";
import ReviewCard from "../components/Ratings/ReviewCard.jsx";
...
import { getCollectorRatings } from "../services/feedbackApi.js";
...
const Collector = () => {
  ...
  const fetchRatingsData = async () => {
    try {
      if (user?._id) {
        const data = await getCollectorRatings(user._id);
        setRatingsData(data);
      }
    } catch (err) {
    ...
    <div className="container">
      ...
      >
        ...
        <div className="card" style={{ padding: 20, display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
          <div style={{ fontSize: 14, color: "#6b7280", fontWeight: 500 }}>Feedback Summary</div>
          {ratingsData?.collector?.ratingCount > 0 ? (
            <>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <StarWidget rating={ratingsData.collector.ratingAvg} interactive={false} size={20} />
                <span style={{ fontSize: 18, fontWeight: 700 }}>{ratingsData.collector.ratingAvg.toFixed(1)}/5</span>
              </div>
              <div style={{ fontSize: 12, color: "#6b7280" }}>{ratingsData.collector.ratingCount} reviews</div>
              {ratingsData.collector.badge && <RatingBadge badge={ratingsData.collector.badge} />}
            </>
          ) : (
            <div style={{ fontSize: 14, color: "#6b7280", textAlign: "center" }}>No feedback available</div>
          )}
          <button
            onClick={() => setShowReviewsDrawer(true)}
            style={{ fontSize: 12, color: "#3b82f6", textDecoration: "underline", cursor: "pointer" }}
          >
            See all reviews
          </button>
        </div>
          ...
          >
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                <h3 style={{ fontSize: 20, fontWeight: 600 }}>Reviews</h3>
                <button onClick={() => setShowReviewsDrawer(false)} style={{ fontSize: 24, background: "none", border: "none", cursor: "pointer" }}>×</button>
              </div>
              <div style={{ marginBottom: 20 }}>
                <Histogram histogram={ratingsData?.histogram || {}} />
              </div>
              <div style={{ display: "grid", gap: 16 }}>
                {ratingsData?.reviews?.map((review) => (
                  <ReviewCard key={review._id} review={review} />
                )) || <div style={{ textAlign: "center", color: "#6b7280" }}>No reviews yet</div>}
              </div>
            </motion.div>
          </motion.div>
        )}
